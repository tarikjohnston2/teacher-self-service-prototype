...
const getPublicUrl = config => {
  if (config.item.endsWith('assets') && config.packageName === 'govuk-frontend') {
    ...
    const prefix = config.type === 'views' ? 'extension-views' : 'extension-assets'
    return ['', prefix, config.packageName]
      .concat(config.item.split('/').filter(filterOutParentAndEmpty))
      .map(s => encodeURIComponent(s).replace('%40', '@').replace('%2F', '/'))
      .join('/')
  }
}
...
const self = module.exports = {
  ...
  getAppConfig: _ => ({
    ...
    views: self.getPublicUrls('views')
  }),
  getAppViews: additionalViews => self
    .getFileSystemPaths('nunjucksPaths')
    .reverse()
    .concat(self.getFileSystemPaths('views').reverse())
