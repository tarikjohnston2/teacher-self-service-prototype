...
const createReleaseArchiveSync = require('./util')
...
describe('create-release-archive/util', () => {
  ...
  describe('getReleaseVersion', () => {
    it('returns the version from package.json if that version has not been released before', () => {
      ...
      expect(
        createReleaseArchiveSync.getReleaseVersion()
      ).toEqual('foo')
    })
    ...
    testFailingIf(process.env.CI, 'tells us the version number of the release described by a ref', () => {
      expect(
        createReleaseArchiveSync.getReleaseVersion('v12.1.0')
      ).toEqual('12.1.0')
    })
  })
  ...
  describe('cleanPackageJson', () => {
    ...
    it('deletes dev dependencies', () => {
      ...
      packageJson = createReleaseArchiveSync.cleanPackageJson(packageJson)
  ...
  describe('updatePackageJson', () => {
    ...
    it('updates a package.json file using the function updater', () => {
      createReleaseArchiveSync.updatePackageJson('package.json', () => { return {} })
      expect(fakeFs['package.json']).toMatch('{}')
    })

    it('preserves the formatting of the package.json file', () => {
      const before = fakeFs['package.json']
      createReleaseArchiveSync.updatePackageJson('package.json', (x) => x)
    ...
    it('runs npm install after changing the file', () => {
      ...
      createReleaseArchiveSync.updatePackageJson('test/package.json', () => { return {} })
  ...
  describe('archiveReleaseFiles', () => {
    ...
    it('zips release files by default', () => {
      if (process.platform === 'win32') {
        createReleaseArchiveSync.archiveReleaseFiles({ cwd: 'C:\\tmp', file: 'C:\\test.zip', prefix: 'test' })
        expect(mockSpawnSync).toBeCalledWith(
          '7z', ['a', '-tzip', '-x!test\\node_modules', 'C:\\test.zip', 'test'],
          expect.objectContaining({ cwd: 'C:\\tmp' })
        )
      } else {
        createReleaseArchiveSync.archiveReleaseFiles({ cwd: '/tmp', file: '/test.zip', prefix: 'test' })
        expect(mockSpawnSync).toBeCalledWith(
    ...
    it('resolves paths correctly for arguments to zip', () => {
      if (process.platform === 'win32') {
        createReleaseArchiveSync.archiveReleaseFiles({ cwd: 'C:\\tmp', file: 'test.zip', prefix: 'test' })
        expect(mockSpawnSync).toBeCalledWith(
    ...
    it('tars release files if file extension is .tar', () => {
      createReleaseArchiveSync.archiveReleaseFiles({ cwd: '/tmp', file: '/test.tar', prefix: 'test' })
    ...
    it('throws an error if file extension is unrecognised', () => {
      expect(
        () => createReleaseArchiveSync.archiveReleaseFiles({ cwd: '/tmp', file: '/test.8z', prefix: 'test' })
      ).toThrow()
    })
  })
})
